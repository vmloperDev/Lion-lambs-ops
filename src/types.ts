...
export type BookingFormData = {
  ...
  // Discount: subtracted from the Invoice and/or Quotation subtotal.
  // discountType: '' (no discount configured), 'percentage' (1-100), or
  // 'fixed' (straight peso/currency amount subtracted from the subtotal).
  discountType: '' | 'percentage' | 'fixed'
  discountValue: string
  // Each document only applies the discount when its own flag is 'true' —
  // set independently via the "Apply to Invoice" / "Apply to Quotation"
  // buttons, so e.g. a discount can be shown on the Quotation to help close
  // the sale but not silently carried over into the final Invoice, or vice versa.
  discountApplyInvoice: 'true' | ''
  discountApplyQuotation: 'true' | ''
}
