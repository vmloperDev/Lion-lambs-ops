...
// ── Discount ─────────────────────────────────────────────────────────────
// A discount is configured once (type + value) in the Data Gathering Form,
// then opted into per-document via discountApplyInvoice / discountApplyQuotation
// so it can show on one document without silently affecting the other.
export function getDiscountAmount(booking: BookingFormData, subtotal: number): number {
  const rawValue = parseAmount(booking.discountValue)
  if (!booking.discountType || !rawValue || rawValue <= 0 || subtotal <= 0) return 0
  if (booking.discountType === 'percentage') {
    const pct = Math.min(Math.max(rawValue, 0), 100)
    return Math.round(subtotal * (pct / 100) * 100) / 100
  }
  // Fixed amount: straight subtraction, never more than the subtotal itself.
  return Math.min(rawValue, subtotal)
}

export function getInvoiceDiscountAmount(booking: BookingFormData, subtotal: number): number {
  return booking.discountApplyInvoice === 'true' ? getDiscountAmount(booking, subtotal) : 0
}

export function getQuotationDiscountAmount(booking: BookingFormData, subtotal: number): number {
  return booking.discountApplyQuotation === 'true' ? getDiscountAmount(booking, subtotal) : 0
}

// Invoice total after discount — the single source of truth for the amount
// actually due, used by the printed Invoice as well as the payment-tracking
// screens (Update Payment / Invoice Editor) so the balance always lines up.
export function getBookingInvoiceTotal(booking: BookingFormData): number {
  const subtotal = getBookingClientTotal(booking)
  return Math.max(subtotal - getInvoiceDiscountAmount(booking, subtotal), 0)
}
